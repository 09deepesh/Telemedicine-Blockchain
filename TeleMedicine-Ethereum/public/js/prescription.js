...
async function handlePrescriptionSubmission(event) {
    ...
    // Generate a digital signature (simplified for demo)
    // In a real application, this would be a proper digital signature
    const signature = web3.utils.sha3(
        `${appointmentId}-${medicationDetails}-${dosage}-${currentAccount}-${Date.now()}`
    );
    
    try {
        const prescriptionId = await issuePrescription(
            appointmentId,
            medicationDetails,
            dosage,
            frequency,
